...
def post(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('You comment has been published.', 'dark')
    ...
    return render_template('main/post.html', posts=[post], comment_pagination=pagination, comments=comments, form=form)
...
def edit(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('The post has been updated.', 'dark')
...
def follow(username):
    ...
    db.session.commit()
    flash('You are now following %s.' % username, 'dark')
    return redirect(url_for('.user', username=username))
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        ...
        rdrct('.user', username=username)
    current_user.unfollow(user)
    db.session.commit()
    flash('You have unfollowed %s.' % username, 'dark')
    return redirect(url_for('.user', username=username))
...
def followed_by(username):
    ...
    follows = [{'user': item.followed, 'timestamp': item.timestamp}
                for item in pagination.items]
    return render_template('main/followers.html', user=user, title='Followed by', endpoint='.followed_by', pagination=pagination, follows=follows)
...
def moderate():
    ...
    return render_template('main/moderate.html', comments=comments,
        comment_pagination=pagination, page=page)
...
def edit_profile_admin(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('The profile has been updated.', 'dark')
